...
FMApp.controller('FileManagerCtr', ['$scope', '$http', '$location',
  function ($scope, $http, $location) {
    ...
    $scope.diskspace = {free:"0",total:"0"};
    ...
    var hash2paths = function (relPath) {
      ...
      for (var i=0; i<names.length; ++i) {
        ...
        if (name) {
          path = path + name + '/';
          paths.push({name: decodeURIComponent(name), path: path});
        }
      }
      return paths;
    };
    ...
    var setCurFiles = function (relPath) {
      $http.get('api' + relPath)
        .success(function (data) {
          $scope.diskspace.free = humanSize(data.info.free);
          $scope.diskspace.total = humanSize(data.info.total);
    ...
    var httpRequest = function (method, url, params, data, config) {
      ...
      conf.params.adminPwd = FM.adminPwd;
      for (var k in config) {
    ...
    FM.btnDisabled = function (btnName) {
      if (FM.adminPwd.length < 5 && btnName != 'download')
       return true;
      switch (btnName) {
        case 'download':
